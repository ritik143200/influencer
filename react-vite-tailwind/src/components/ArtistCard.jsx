        ...
        >
          ...
          <div className="flex-1 min-w-0 md:pr-0">
            ...
            <div className="flex flex-col md:flex-row md:flex-wrap lg:flex-nowrap md:items-start md:gap-0 mb-4 min-w-0">
                ...
                {artistSubcategory && (
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-gradient-to-r from-brand-50 to-gray-50 border border-brand-100 text-brand-700 text-xs font-medium rounded-md flex items-center gap-1 flex-shrink-0">
              ...
              {/* Price and Action */}
              <div className="w-full mt-4 md:mt-0 md:w-full lg:w-[220px] lg:flex-shrink-0 md:self-start min-w-0">
                <div className="text-left md:text-right mb-3">
                  ...
                  <div className="text-2xl sm:text-3xl font-bold text-brand-600 break-words max-w-full">
                    {artistPrice}
                  </div>
                </div>
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <button 
                    ...
                    className="w-full px-4 py-2 border-2 border-brand-500 text-brand-600 rounded-xl hover:bg-brand-50 transition-colors font-medium flex-shrink-0"
                  >
                    Contact
                  </button>
                </div>
      ...
      >
        <div className="relative" style={{ aspectRatio: '1/1' }}>
          <div
            ...
            style={{
              background: `linear-gradient(140deg, ${config.primary_action}14 0%, ${config.surface_color} 60%, ${config.secondary_action}14 100%)`
            }}
          >
            {isImageURL ? (
              <img
          ...
          <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5">
            {artist.trending && (
              <span
                className="px-2.5 py-1 rounded-full text-white text-[11px] font-semibold"
                style={{
                  backgroundColor: config.primary_action,
                  boxShadow: `0 10px 18px -14px ${config.primary_action}`
                }}
              >
                🔥 Trending
              </span>
            ...
            {isVerified && (
              <span
                ...
                style={{
                  ...
                  backgroundColor: `${config.surface_color}E8`
                }}
              >
                ✓ Verified
              </span>
        ...
        <div className="p-3.5">
          <h3 className="font-bold text-sm truncate" style={{ color: config.text_color }}>
            {artistName}
          </h3>
          <p className="text-xs mt-0.5 truncate" style={{ color: config.secondary_action }}>
            {artistSpecialty}
          </p>

          <div className="flex items-center justify-between gap-2 mt-3 mb-2.5">
            <span
              className="px-2 py-1 text-[10px] uppercase tracking-wide font-semibold rounded-full border truncate"
              style={{
                color: config.text_color,
                borderColor: `${config.secondary_action}2D`,
            ...
            <div className="flex items-center gap-1 text-xs">
              ...
              <span className="font-semibold" style={{ color: config.text_color }}>{artistRating.toFixed(1)}</span>
              <span style={{ color: config.secondary_action }}>({artistReviews})</span>
            </div>
          </div>

          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] uppercase tracking-wide" style={{ color: config.secondary_action }}>
              Starting
            </span>
            <span className="font-bold text-sm truncate" style={{ color: config.primary_action }}>
              {artistPrice}
            </span>
          </div>
      ...
      <div className="p-3 pb-0">
        <div className="relative rounded-2xl overflow-hidden" style={{ aspectRatio: '1/1.22' }}>
          <div
            ...
            style={{
              background: `linear-gradient(145deg, ${config.primary_action}16 0%, ${config.surface_color} 55%, ${config.secondary_action}12 100%)`
            }}
          >
            {isImageURL ? (
              <img
          ...
          <div className="absolute top-3 left-3 flex items-center gap-2 flex-wrap">
            {artist.trending && (
              <div
                ...
                style={{
                  backgroundColor: config.primary_action,
                  boxShadow: `0 10px 18px -12px ${config.primary_action}`
                }}
              >
                🔥 Trending
              </div>
      ...
      <div className="p-4 pt-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="min-w-0">
            <h3 className="font-bold text-base truncate" style={{ color: config.text_color }}>{artistName}</h3>
            <p className="text-sm truncate" style={{ color: config.secondary_action }}>{artistSpecialty}</p>
          </div>
          <span
            className="px-2.5 py-1 text-[11px] font-semibold rounded-full border whitespace-nowrap"
            style={{
              color: config.text_color,
              borderColor: `${config.secondary_action}35`,
              backgroundColor: `${config.surface_color}CC`
            }}
          >
            {artistCategory}
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div
            ...
            style={{
              ...
              backgroundColor: `${config.primary_action}0D`
            }}
          >
            <p className="text-[10px] uppercase tracking-wide mb-1" style={{ color: config.secondary_action }}>
              Rating
            </p>
            <div className="flex items-center gap-1">
              <span className="text-yellow-500 text-sm">⭐</span>
              <span className="font-semibold text-sm" style={{ color: config.text_color }}>{artistRating.toFixed(1)}</span>
              <span className="text-[11px]" style={{ color: config.secondary_action }}>({artistReviews})</span>
            </div>
          </div>
          ...
          >
            ...
            <p className="font-bold text-sm truncate" style={{ color: config.primary_action }}>
              {artistPrice}
            </p>
          </div>
        </div>
